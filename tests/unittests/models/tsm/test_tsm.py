...
class TSMTest(unittest.TestCase):
    def test_tsm_resnet_rgb(self):
        ...
        self.assertTrue(out.shape == torch.Size([2,2]))
    ...
    def test_tsm_resnet_flow(self):
        ...
        non_local = True
        pretrain = False
        input_tensor=torch.randn(2,10,32,32)
        model = TSN(num_class = num_class,
                    num_segments = num_segments,
                    modality = modality,
                    is_shift = is_shift,
    ...
    def test_tsm_resnet_diff(self):
        ...
        non_local = True
        pretrain = False
        input_tensor=torch.randn(54,1,32,32)
        model = TSN(num_class = num_class,
                    num_segments = num_segments,
                    modality = modality,
                    is_shift = is_shift,
                    ...
                    pretrain = pretrain)
        out = model(input_tensor)
        self.assertTrue(out.shape == torch.Size([1,2]))
