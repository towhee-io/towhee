...
class TestPyOpToTriton(unittest.TestCase):
    ...
    def test_py_to_triton(self):
        with TemporaryDirectory(dir='./') as root:
            ...
            to_triton = PyOpToTriton(op, root, 'e4b074eb_local_triton_py', 'local', 'triton_py', {}, {'device_ids': [1]})
            to_triton.to_triton()
...
class TestPreprocessor(unittest.TestCase):
    ...
    def test_processor(self):
        with TemporaryDirectory(dir='./') as root:
            ...
            to_triton = PreprocessToTriton(op, root, 'db5377c3_local_triton_nnop_preprocess',
                                           {
                                               'dynamic_batching': {
                                                   'max_batch_size': 128,
...
class TestPostprocessor(unittest.TestCase):
    ...
    def test_processor(self):
        with TemporaryDirectory(dir='./') as root:
            op = ops.local.triton_nnop(model_name='test').get_op()
            to_triton = PostprocessToTriton(op, root, 'db5377c3_local_triton_nnop_postprocess',
                                            {
                                               'dynamic_batching': {
                                                   'max_batch_size': 128,
                                                   'preferred_batch_size': [1, 2],
...
class TestToModel(unittest.TestCase):
    ...
    def test_to_model(self):
        with TemporaryDirectory(dir='./') as root:
            op = ops.local.triton_nnop(model_name='test').get_op()
            to_triton = ModelToTriton(op, root, 'db5377c3_local_triton_nnop_model', ['tensorrt'],
                                           {
                                               'device_ids': [1, 2],
                                               'instance_count': 2,
                                               'dynamic_batching': {
...
class TestToEnsemble(unittest.TestCase):
    ...
    test_data = {
        ...
        'fae9ba13_local_triton_nnop_postprocess': {
            ...
            'child_ids': [],
            'parent_ids': ['fae9ba13_local_triton_nnop_model']
        }
    }
