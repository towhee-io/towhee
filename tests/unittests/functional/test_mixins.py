...
class TestRemoteMixin(unittest.TestCase):
    ...
    def test_remote(self):
        dc = DataCollection(((1, 2, 3, 4, 5, 6), (2, 3, 4, 5, 6, 7)))
        remote = dc.remote('127.0.0.1:8001', mode='infer', protocol='grpc')
        self.assertEqual(remote[0], None)

    def test_remote_error(self):
        dc = DataCollection(((1, 2, 3, 4, 5, 6), (2, 3, 4, 5, 6, 7)))
        remote = dc.remote('127.0.0.1:8000', mode='async_infer', protocol='http')
        self.assertEqual(remote[0], None)
