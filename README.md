...
- __Pipeline__: A pipeline is composed of several operators. Operators are connected together as a DAG(directed acyclic graph) to create more complex functionalities, such as embedding feature extraction, data tagging, cross modal data understanding, etc.

- __DataCollection__: A pythonic and method-chaining style API that for building custom pipelines. Pipelines defined by DataColltion API can be either run on notebook localy for fast prototyping, or converting to image docker with end-to-end optimization for production-ready environments. 
