...
class DataFrameIterator:
    ...
    def accessible_size(self) -> int:
        ...
        """
        return len(self._df_ref()) - self._offset
...
class WindowIterator(DataFrameIterator):
    ...
    def __next__(self):
        ...
        if code in (Responses.EMPTY_SEALED, Responses.FUTURE_WINDOW_SEALED):
            ...
            df.notify_window_block(self._id, self._event, 'end', (self._comparator, self._current_window[1]))
            self._event.wait()
            self._event.clear()
            return self.__next__()

        elif code == Responses.FUTURE_WINDOW:
            df.ack(self._id, offset) #TODO: Garbage collection logic if waiting on window that is far.
            df.notify_window_block(self._id, self._event, 'start', (self._comparator, self._current_window[0]))
            self._event.wait()
            self._event.clear()
            return self.__next__()
