...
def multi_head_attention_forward(query: Tensor,
    ...
    if not torch.jit.is_scripting():
        ...
        if any(not isinstance(t, Tensor) for t in tens_ops) and F.has_torch_function(tens_ops):
    ...
    if not use_separate_proj_weight:
        if torch.equal(query, key) and torch.equal(key, value):
            ...
            if key is None:
                ...
                start_ = embed_dim
                end_ = None
                w_ = in_proj_weight[start_:, :]
                if b_ is not None:
                    b_ = b_[start_:]
                k, v = F.linear(key, w_, b_).chunk(2, dim=-1)
            ...
            if b_ is not None:
                b_ = b_[start_:]
            v = F.linear(value, w_, b_)
    else:
        q_proj_weight_non_opt = torch.jit._unwrap_optional(q_proj_weight)  # pylint: disable=protected-access
        len1, len2 = q_proj_weight_non_opt.size()
        assert len1 == embed_dim and len2 == query.size(-1)

        k_proj_weight_non_opt = torch.jit._unwrap_optional(k_proj_weight)  # pylint: disable=protected-access
        len1, len2 = k_proj_weight_non_opt.size()
        assert len1 == embed_dim and len2 == key.size(-1)

        v_proj_weight_non_opt = torch.jit._unwrap_optional(v_proj_weight)  # pylint: disable=protected-access
