...
def get_repo_obj(args):
    if args.command == 'download':
        ...
        repo_obj = GitUtils(args.author, args.repo, repo_path=Path(args.dir)/args.repo)
...
def init_repo(args, dir):
    repo_manager = get_repo_obj(args)
    repo_path = dir / args.repo
    GitUtils(args.author, args.repo, repo_path=repo_path).clone()
    if args.classes == 'pipeline':
        temp_path = dir / 'pipeline-template'
        GitUtils('towhee', 'pipeline-template', repo_path=temp_path).clone()
        repo_manager.init_pipeline(temp_path, repo_path)
    elif args.classes == 'pyoperator':
        temp_path = dir / 'pyoperator-template'
        GitUtils('towhee', 'pyoperator-template', repo_path=temp_path).clone()
        repo_manager.init_pyoperator(temp_path, repo_path)
    elif args.classes == 'nnoperator':
        temp_path = dir / 'nnoperator-template'
        GitUtils('towhee', 'nnoperator-template', repo_path=temp_path).clone()
...
def main():
    ...
    subparsers.add_parser('init', parents=[ar_parser, c2_parser, d_parser, f_parser], add_help=False, description='Initialize the file for your Repo.')
