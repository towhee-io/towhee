...
class DagMixin:
    ...
    def notify_consumed(self, new_id):
        info = {'op': 'nop', 'init_args': None, 'call_args': None, 'parent_ids': self.parent_ids, 'child_ids':  [new_id]}
        self.control_plane.dag[self.id] = info

    def compile_dag(self):
        info = {'op': 'nop', 'init_args': None, 'call_args': None, 'parent_ids': self.parent_ids, 'child_ids':  ['end']}
        self.control_plane.dag[self.id] = info
        info = {'op': 'end', 'init_args': None, 'call_args': None, 'parent_ids': [self.id], 'child_ids':  []}
