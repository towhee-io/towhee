...
class OperatorPool:
    ...
    @staticmethod
    def _operator_id(hub_op_id: str, op_args: List[any], op_kwargs: Dict[str, any], tag: str):
        if op_kwargs:
            kwargs_tuple = tuple(sorted(op_kwargs.items()))
        else:
            kwargs_tuple = ()
        if op_args:
            args_tuple = tuple(sorted(op_args))
        else:
            args_tuple = ()
        return (hub_op_id, tag) + args_tuple + kwargs_tuple
    ...
    def acquire_op(self, hub_op_id: str, op_args: List[any], op_kwargs: Dict[str, any], tag: str) -> Operator:
        """
        ...
        # dictionary.
        #TODO: Add op_args to key.
        op_key = OperatorPool._operator_id(hub_op_id, op_args, op_kwargs, tag)
        with self._lock:
            ...
            if not storage.op_available():
                #TODO: Add op_args to loader.
                op = self._op_loader.load_operator(hub_op_id, op_args, op_kwargs, tag)
                op.key = op_key
                storage.put(op, True)
            return storage.get()
