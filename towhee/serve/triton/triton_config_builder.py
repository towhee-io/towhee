                        ...
                        if  'input_info' in j:
                            ...
                            for v in j['input_info']:
                                ...
                                if pid == 'start':
                                    ...
                                    indx = self._dag[pid]['output_info'].index(v[1])
                                    schema = self._dag[pid]['output_schema'][indx]
                                    pid_output_indx = find_output_index(schema_list, schema)
                                    for inx in pid_output_indx:
                                        input_key = 'INPUT'+str(k)
                                        input_map.append((input_key, '_'.join([str(pid), inx])))
                                        k+=1
