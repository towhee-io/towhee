...
class GitUtils:
    ...
    """
    def __init__(self, author: str, repo: str, root: str = 'https://hub.towhee.io', repo_path: Union[str, Path] = None):
        self._author = author
        self._repo = repo
        self._root = root
        self._repo_file_name = repo.replace('-', '_')
        if not repo_path:
            repo_path = Path.cwd() / self._repo_file_name
    ...
    def clone(self, tag: str = 'main', install_reqs: bool = True) -> None:
        """
        ...
        if not self._repo_path:
            self._repo_path = Path.cwd() / repo_file_name

        if isinstance(self._repo_path, str):
            self._repo_path = Path(self._repo_path)
        ...
        subprocess.check_call(['git', 'clone', '-b', tag, url, self._repo_path])

        if install_reqs:
            if 'requirements.txt' in os.listdir(self._repo_path):
                subprocess.check_call([sys.executable, '-m', 'pip', 'install', '-r', self._repo_path / 'requirements.txt'])
